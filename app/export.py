...
def build_margin_analysis(analysis: dict) -> str:
    """Internal cost/profit sheet, in the same shape as a customer quotation.

    Deliberately a thin wrapper over build_xls_minimal rather than a second
    builder: the layout, the image embedding and the totals are the parts
    worth reusing, and duplicating them would leave two sheets to keep in
    step. Cost / Profit / Margin are APPENDED as columns N/O/P past the
    template's own A..M, the same tactic build_xls_from_template uses for its
    BOQ columns — nothing in the existing layout has to move.

    The result carries purchase cost, so it is titled and watermarked as
    internal; the endpoint that serves it is admin-only.
    """
    lines = analysis.get("lines") or []
    items = [{
        "product":       l.get("matched_to") or l.get("product") or "",
        "qty":           int(l.get("qty") or 0),
        "price_per_pc":  float(l.get("sold") or 0),
        "model_no":      l.get("model_no") or "",
        "brand":         l.get("brand") or "",
        "specification": l.get("specification") or "",
        "description":   l.get("product") or "",       # what the client's file called it
        "hsn_code":      l.get("hsn_code") or "",
        "image_path":    l.get("image_path") or "",
        "gst_pct":       l.get("gst_pct") if l.get("gst_pct") is not None else 18,
    } for l in lines]

    src_name = re.sub(r"[^A-Za-z0-9_-]+", "-", (analysis.get("filename") or "quotation")).strip("-")
    path = Path(build_xls_minimal({"ref_no": f"MARGIN-{src_name}", "client_name": "— internal analysis —"},
                                  items))

    wb = openpyxl.load_workbook(str(path))
    ws = wb.active
    NAVY, LIGHT, WHITE, RED = "1F3864", "DCE6F1", "FFFFFF", "C00000"
    money, pct = "₹#,##0.00", '0.0"%"'

    def put(r, c, v, *, bold=False, size=9, color="000000", bg=None, align="right", fmt=None):
        cl = ws.cell(r, c, v)
        cl.font = Font(bold=bold, size=size, color=color, name="Calibri")
        cl.alignment = Alignment(horizontal=align, vertical="center")
        if bg:  cl.fill = PatternFill("solid", start_color=bg)
        if fmt: cl.number_format = fmt
        return cl

    # retitle: this is not a document anybody should send out
    ws.cell(4, 1).value = "M A R G I N   A N A L Y S I S   —   I N T E R N A L"
    ws.cell(10, 1).value = ("INTERNAL ONLY — contains purchase cost. Do not send this file to a customer. "
                            f"Source file: {analysis.get('filename') or '—'}")
    ws.cell(10, 1).font = Font(bold=True, size=9, color=RED, name="Calibri", italic=True)

    COST_C, PROFIT_C, MARGIN_C = 14, 15, 16
    for c, h, w in ((COST_C, "COST/PC (₹)", 14), (PROFIT_C, "PROFIT (₹)", 14), (MARGIN_C, "MARGIN %", 11)):
        put(12, c, h, bold=True, color=WHITE, bg=NAVY, align="center")
        ws.column_dimensions[get_column_letter(c)].width = w

    for i, l in enumerate(lines):
        r = 13 + i
        bg = "F2F2F2" if i % 2 else WHITE
        put(r, COST_C,   l.get("cost"),   bg=bg, fmt=money)
        put(r, PROFIT_C, l.get("profit"), bg=bg, fmt=money,
            bold=True, color="008000" if (l.get("profit") or 0) >= 0 else RED)
        put(r, MARGIN_C, l.get("margin_pct"), bg=bg, fmt=pct)

    # totals, aligned to the SUB TOTAL row the base builder already wrote
    for r in range(13 + len(lines), ws.max_row + 1):
        if str(ws.cell(r, 1).value or "").strip().upper() == "SUB TOTAL":
            put(r, COST_C,   "TOTAL", bold=True, size=10, color=NAVY, bg=LIGHT, align="center")
            put(r, PROFIT_C, analysis.get("profit"), bold=True, size=10, bg=LIGHT, fmt=money)
            put(r, MARGIN_C, analysis.get("margin_pct"), bold=True, size=10, bg=LIGHT, fmt=pct)
            break

    counts = analysis.get("counts") or {}
    note = ws.max_row + 2
    ws.cell(note, 1, f"Profit and margin cover only the {counts.get('ok', 0)} line(s) with a known master cost "
                     f"({counts.get('no_cost', 0)} have no cost, {counts.get('not_in_master', 0)} are unmatched). "
                     f"Quoted value ₹{analysis.get('revenue', 0):,.2f}, of which ₹{analysis.get('known_revenue', 0):,.2f} is costed.")
    ws.cell(note, 1).font = Font(size=8, italic=True, color="666666", name="Calibri")

    out = EXPORTS_DIR / f"Margin_{src_name}.xlsx"
    wb.save(str(out))
    path.unlink(missing_ok=True)          # the customer-shaped intermediate
    return str(out)
